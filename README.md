...
- 所有自动组使用 HTTPS 探测地址，测试间隔 300 秒，容差 `0 ms`，严格选择当前最低延迟节点。
- 海外应用默认不提供 `DIRECT`，集中通过 `🔒 隐私代理`、地区组或手动节点出站。
...
- 若最低延迟节点切换过于频繁，把 `upstream/metafenliu.ini` 中自动组末尾的容差从 `0` 改为 `30`，然后重新生成。
