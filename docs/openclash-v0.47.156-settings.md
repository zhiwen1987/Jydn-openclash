...
5. 打开 `modules/openclash-dns-privacy-override.yaml`，从 `[YAML]` 复制到最后一行，粘贴到模块编辑框。
...
国内连接会先进入 Mihomo，再由 `GEOSITE,cn` / `GEOIP,cn` 决定 `DIRECT`。该开关属于插件级持久设置，不在 YAML 覆写模块中重复写入。
...
1. 打开 **覆写设置 → Smart 设置**。
2. 开启 **Smart 策略自动切换**，并确认版本页使用 **Smart 内核**。
3. 开启 **LightGBM** 和模型自动更新，更新间隔设为 **72 小时**。
4. **Policy Priority** 设为 `0`，**Prefer-ASN** 和 **训练数据采集**关闭。
5. Smart 容差设为 **100 ms**，点击 **应用设置**。

模板先生成 43 个 `url-test` 节点池，覆写模块再将其转换为 Smart：使用 `https://cp.cloudflare.com/generate_204`、120 秒间隔、100 ms 防抖容差、5 秒超时、失败阈值 `2` 和 HTTP `204` 校验。除健康检查外，Smart 还会按真实目标站点的连接延迟、失败和历史表现重新选路。`🛟 稳定自动` 继续优选日本/新加坡 CUCM、专线流媒体线路，并以 `♻️ 自动选择` 作为兜底。

插件级选项必须在 LuCI 中保存。当前版本若在模块 `[General]` 中重复写入，匿名 UCI 覆写项可能在启动时被错误当作 shell 变量执行；本仓库因此让模块只负责 YAML，避免重启出现 `not found`。

## 9. WebRTC 与浏览器独立 DNS

- `🛡️ 实时通信（WebRTC）` 默认选择 `REJECT`，阻断常见 STUN/TURN 端口 3478-3481 和 5349；这可能影响网页会议和语音通话。
