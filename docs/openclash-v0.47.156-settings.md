...
模板里的八个节点池均为 `url-test`，使用 `https://cp.cloudflare.com/generate_204`、120 秒间隔和 `10 ms` 容差。`🛟 稳定自动` 优选日本/新加坡 CUCM、专线流媒体线路，并以 `♻️ 自动选择` 作为非直连兜底。覆写模块额外设置 5 秒超时、失败阈值 `2` 和预期 HTTP 状态 `204`；若需要严格追随当前最低延迟，可将容差改为 `0 ms`。
