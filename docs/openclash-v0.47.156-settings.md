...
模板先生成八个 `url-test` 节点池，覆写模块再将其转换为 Smart：使用 `https://cp.cloudflare.com/generate_204`、120 秒间隔、100 ms 防抖容差、5 秒超时、失败阈值 `2` 和 HTTP `204` 校验。除健康检查外，Smart 还会按真实目标站点的连接延迟、失败和历史表现重新选路。`🛟 稳定自动` 继续优选日本/新加坡 CUCM、专线流媒体线路，并以 `♻️ 自动选择` 作为兜底。
